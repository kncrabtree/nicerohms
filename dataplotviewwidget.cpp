...
DataPlotViewWidget::DataPlotViewWidget(QWidget *parent) :
    QWidget(parent)
{    
    QSettings s;
    int numPlots = qBound(1,s.value(QString("dataPlotWidget/numPlots"),4).toInt(),9);
...
DataPlotViewWidget::~DataPlotViewWidget()
{

}

void DataPlotViewWidget::initializeForExperiment()
{
    d_plotCurves.clear();
...
void DataPlotViewWidget::curveVisibilityToggled(QwtPlotCurve *c, bool visible)
{
    ...
    QSettings s;
    s.setValue(QString("dataPlotWidget/curves/%1/isVisible").arg(d_plotCurves.at(i).name),visible);
    s.sync();
...
void DataPlotViewWidget::curveContextMenuRequested(QwtPlotCurve *c, QMouseEvent *me)
{
...
void DataPlotViewWidget::changeCurveColor(int curveIndex)
{
    ...
    if(newColor.isValid())
    {
        ...
        QSettings s;
	   s.setValue(QString("dataPlotWidget/curves/%1/color").arg(d_plotCurves.at(curveIndex).name),newColor);
        s.sync();
...
void DataPlotViewWidget::moveCurveToPlot(int curveIndex, int newPlotIndex)
{
    ...
    QSettings s;
    s.setValue(QString("dataPlotWidget/curves/%1/plotIndex").arg(d_plotCurves.at(curveIndex).name),
               newPlotIndex);
    s.sync();
...
void DataPlotViewWidget::changeCurveAxis(int curveIndex)
{
    if(curveIndex < 0 || curveIndex > d_plotCurves.size())
        return;
    ...
    QSettings s;
    s.setValue(QString("dataPlotWidget/curves/%1/axis").arg(d_plotCurves.at(curveIndex).name),newAxis);
    s.sync();
...
void DataPlotViewWidget::pushXAxis(int sourcePlotIndex)
{
...
void DataPlotViewWidget::autoScaleAll()
{
    for(int i=0; i<d_allPlots.size(); i++)
        d_allPlots.at(i)->autoScale();
}

void DataPlotViewWidget::changeNumPlots()
{
    bool ok = true;
    ...
    QSettings s;
    s.setValue(QString("dataPlotWidget/numPlots"),newNum);
...
int DataPlotViewWidget::findCurveIndex(QwtPlotCurve *c)
{
    if(c == nullptr)
        return -1;
...
void DataPlotViewWidget::addNewPlot()
{
    DataPlot *tp = new DataPlot(QString("DataPlot%1").arg(d_allPlots.size()),this);
    ...
    connect(tp,&DataPlot::curveVisiblityToggled,this,&DataPlotViewWidget::curveVisibilityToggled);
    connect(tp,&DataPlot::legendItemRightClicked,this,&DataPlotViewWidget::curveContextMenuRequested);
    int newPlotIndex = d_allPlots.size();
    connect(tp,&DataPlot::axisPushRequested,this,[=](){ pushXAxis(newPlotIndex); });
    connect(tp,&DataPlot::autoScaleAllRequested,this,&DataPlotViewWidget::autoScaleAll);

    d_allPlots.append(tp);

}

void DataPlotViewWidget::configureGrid()
{
...
void DataPlotViewWidget::setAutoScaleYRanges(int plotIndex, QwtPlot::Axis axis)
{
    double min;
    double max;
    bool foundCurve = false;
