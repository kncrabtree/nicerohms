...
void AomSynthesizer::readTripParameters()
{
    ...
    d_lowTrip = s.value(QString("%1/%2/lowTrip").arg(d_key).arg(d_subKey),150e6).toDouble();
    d_highTrip = s.value(QString("%1/%2/highTrip").arg(d_key).arg(d_subKey),200e6).toDouble();
}
